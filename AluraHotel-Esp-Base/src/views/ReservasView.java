...
public class ReservasView extends JFrame {
	...
	private void guardarReserva() {
		if (txtFechaEntrada.getDate() != null && txtFechaSalida.getDate() != null && !txtValor.equals("") 
			...
			LocalDate dataEntrada = LocalDate.parse(((JTextField)txtFechaEntrada.getDateEditor().getUiComponent()).getText());
			LocalDate dataSalida = LocalDate.parse(((JTextField)txtFechaEntrada.getDateEditor().getUiComponent()).getText());
            Reserva reserva = new Reserva (dataEntrada, dataSalida, txtValor.getText(),txtFormaPago.getSelectedItem().toString());
	...
	public void calcularValor(JDateChooser dataEntrada, JDateChooser dataSalida) {
		if(dataEntrada.getDate()!= null && dataSalida.getDate() !=null ) {
			if(dataEntrada.getDate().after(dataSalida.getDate())) {
				JOptionPane.showMessageDialog(null, "La fecha de entrada no puede ser después  a la fecha de salida",
			"Error en fechas", JOptionPane.ERROR_MESSAGE);
				return;
			}
			Calendar inicio = dataEntrada.getCalendar();
			Calendar fin = dataSalida.getCalendar();
            int dias = -1;
            int noche = 50;
            int valor;
